...
func Decode(r tiff.ReadAtReaderSeeker) (*Exif, error) {
	...
	if isTiff {
		...
		var hr tiff.ReadAtReaderSeeker
		hr, err = newHEICReader(r)
		if err != nil {
			return nil, err
		}
		tif, err = tiff.Decode(hr)
		if err != nil {
			return nil, err
		}
		hr.Seek(0, 0)
...
func newHEICReader(r io.Reader) (*bytes.Reader, error) {
	...
	buf := make([]byte, extentLength)
	_, err = io.ReadFull(io.LimitReader(exifReader, int64(extentLength)), buf)
	if err != nil {
		return nil, err
	}
